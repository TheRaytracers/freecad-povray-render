...
class PointLight:
    ...
    def setProperties(self, obj):
        if not "Color" in obj.PropertiesList:
            obj.addProperty(
                "App::PropertyColor", "Color", "Light", "Color of the Light"
            )
...
class ViewProviderPointLight:
    ...
    def attach(self, obj):
        ...
        img = coin.SoImage()
        img.filename = os.path.join(
            os.path.dirname(__file__), "icons", "pointLight.svg"
        )
    ...
    def setProperties(self, obj):
        ...
        if not "FadePower" in obj.PropertiesList:
            obj.addProperty(
                ...
                "Light",
                "Potency of light decrease (2=quadratic, 3=cubic, etc.)",
            ).FadePower = 0  # XXX negative values are not allowed
        ...
        if not "WidthLights" in obj.PropertiesList:
            obj.addProperty(
                ...
                "WidthLights",
                "AreaLight",
                "Number of lights in width axis",
            ).WidthLights = (5, 1, 9999, 1)
        if not "LengthLights" in obj.PropertiesList:
            obj.addProperty(
                "App::PropertyIntegerConstraint",
                "LengthLights",
                "AreaLight",
                "Number of lights in length axis",
            ).LengthLights = (5, 1, 9999, 1)
        ...
        if not "Adaptive" in obj.PropertiesList:
            obj.addProperty(
                ...
                "Adaptive",
                "AreaLight",
                "The higher the number, the better the rendering result, but also the slower",
            ).Adaptive = (1, 0, 9999, 1)
        if not "Jitter" in obj.PropertiesList:
            obj.addProperty(
                "App::PropertyBool", "Jitter", "AreaLight", "Random shadow softening"
            ).Jitter = True

        if not "AreaIllumination" in obj.PropertiesList:
            obj.addProperty(
                "App::PropertyBool", "AreaIllumination", "AreaLight", "TOOLTIP NEEDED"
            ).AreaIllumination = False
        ...
        for i in range(self.lengthLights):
            for k in range(self.widthLights):
                ...
                trans.translation.setValue(
                    [-self.length / 2 + x * i, -self.width / 2 + y * k, 0]
                )
                ...
                img.vertAlignment = img.CENTER
                img.horAlignment = img.CENTER
                img.width = App.ParamGet(preferences.prefPath).GetInt(
                    "LightIconSize", 16
                )
                img.height = App.ParamGet(preferences.prefPath).GetInt(
                    "LightIconSize", 16
                )
    ...
    def setProperties(self, obj):
        ...
        if not "FallOff" in obj.PropertiesList:
            obj.addProperty(
                ...
                "FallOff",
                "SpotLight",
                "Angle of the outer light circle",
            ).FallOff = 45
        if not "Tightness" in obj.PropertiesList:
            obj.addProperty(
                ...
                "Tightness",
                "SpotLight",
                "Exponential softening of the edges (higher = sharper)",
            ).Tightness = (0, 0, 100, 1)
...
class ViewProviderSpotLight:
    def __init__(self, obj):
        ...
        obj.addProperty(
            ...
            "ShowCones",
            "SpotLight",
            "Show the cones, that are representing the light",
        )
        obj.ShowCones = True
